...
import { AgendaAssetFilterForm } from './agenda-asset-filter-form';
      ...
      <Panel>
        <div className="agenda-calendar-chrome">
          ...
          <div className="agenda-calendar-toolbar">
            <AgendaAssetFilterForm
              assets={fleetAssets.map((asset) => ({
                id: asset.id,
                label: formatAgendaAssetName(asset.name)
              }))}
                            ...
                            {lane.map((segment) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span>{formatAgendaAssetName(segment.event.assetName)}</span>
                                </Link>
                              );
                            })}
                          </div>
              ...
              <div className="form-grid">
                ...
                {canChooseCreateAsset ? (
                    ...
                    <select name="assetId" defaultValue={defaultCreateAssetId}>
                      {fleetAssets.map((asset) => (
                        <option key={asset.id} value={asset.id}>
                          {formatAgendaAssetName(asset.name)}
                        </option>
                      ))}
                    </select>
                  </label>
                  ...
                  <div className="form-field form-field--full">
                    ...
                    <div className="form-value">
                      {defaultCreateAsset ? formatAgendaAssetName(defaultCreateAsset.name) : 'Ativo não informado'}
                    </div>
                  </div>
                )}
                ...
                <label className="form-field form-field--full">
                  ...
                  <textarea
                    name="description"
                    rows={4}
                    placeholder="Descreva a utilização, manutenção ou bloqueio operacional."
                    required
                  />
                </label>
              </div>
                ...
                <form action={rescheduleAgendaEventAction} className="action-form">
                  ...
                  <div className="form-grid">
                    ...
                    {canChooseCreateAsset ? (
                      ...
                      <div className="form-field form-field--full">
                        <span>Ativo</span>
                        <div className="form-value">{formatAgendaAssetName(selectedEvent.assetName)}</div>
            ...
            {isConfirmingDelete ? (
              <div className="modal-inline-confirmation" role="dialog" aria-modal="true">
                <div className="modal-inline-confirmation__card">
                  ...
                  <p>Essa ação remove o evento do calendário do ativo selecionado.</p>
                  <div className="form-actions">
                    <Link
                      className="action-button action-button--ghost"
                      href={buildAgendaViewHref(monthCursor, {
...
function buildCalendarCells(events: AgendaEventRecord[], monthCursor: AgendaMonthCursor) {
  ...
  for (const event of events) {
    ...
    while (current <= rangeEndDate) {
      ...
      dayEvents.sort((left, right) => {
        const startDelta = Date.parse(left.startsAt) - Date.parse(right.startsAt);
  ...
  for (let index = 0; index < startOffset; index += 1) {
    const date = new Date(Date.UTC(monthCursor.year, monthCursor.monthIndex, 1 - startOffset + index, 12));

    cells.push({
      key: `outside-start-${index}`,
      inMonth: false,
      dayLabel: '',
      events: [],
      date
...
function buildWeekCalendarEventSegments(
  ...
  for (const event of events) {
    ...
    const startColumn = differenceInUtcCalendarDays(weekStartTime, overlapStartTime) + 1;
    const endColumn = differenceInUtcCalendarDays(weekStartTime, overlapEndTime) + 1;
  ...
  segments.sort((left, right) => {
    const startDelta = Date.parse(left.event.startsAt) - Date.parse(right.event.startsAt);

    if (startDelta !== 0) {
      return startDelta;
    }

    return left.event.title.localeCompare(right.event.title, 'pt-BR');
  });
...
function buildWeekCalendarEventLanes(
  ...
  for (const segment of sortedSegments) {
    let targetLane = lanes.findIndex((lane) => {
      const lastSegment = lane[lane.length - 1];

      return Boolean(lastSegment && lastSegment.endColumn < segment.startColumn);
    });
...
function buildCalendarEventPresentation(
  ...
  return {
    className: buildCalendarEventClassName(emphasis),
    style: buildAgendaThemeStyle(theme)
  };
}

function isAgendaMultiDayEvent(event: AgendaEventRecord) {
  return toUtcDateKey(new Date(event.startsAt)) !== toUtcDateKey(new Date(event.endsAt));
}

function buildAgendaThemeStyle(
  theme: ReturnType<typeof resolveAgendaEventColorTheme>
): CalendarEventStyle {
  return {
...
function buildCalendarEventClassName(emphasis: AgendaEventEmphasis) {
  const baseClass = 'calendar-event-chip calendar-event-link';

  if (emphasis === 'critical') {
    return `${baseClass} calendar-event-chip--critical`;
  }

  if (emphasis === 'warning') {
    return `${baseClass} calendar-event-chip--warning`;
  }
...
function readAgendaMode(searchParams: Record<string, string | string[] | undefined>) {
  const value = searchParams.mode;

  return value === 'create' || value === 'edit' ? value : undefined;
}

function readAgendaEventId(searchParams: Record<string, string | string[] | undefined>) {
  const value = searchParams.eventId;
...
function readAgendaAssetId(
  ...
  const value = searchParams.assetId;

  if (typeof value !== 'string' || value.trim().length === 0) {
    return undefined;
  }

  return fleetAssets.some((asset) => asset.id === value) ? value : undefined;
}

function readAgendaEditIntent(searchParams: Record<string, string | string[] | undefined>) {
  const value = searchParams.intent;

  return value === 'edit' ? 'edit' : 'view';
}

function readAgendaDeleteConfirmation(searchParams: Record<string, string | string[] | undefined>) {
  const value = searchParams.confirmDelete;

  return value === 'true';
}

function readAgendaMonthCursor(searchParams: Record<string, string | string[] | undefined>) {
...
function shiftAgendaMonth(monthCursor: AgendaMonthCursor, offset: number) {
  return createAgendaMonthCursor(monthCursor.year, monthCursor.monthIndex + offset);
}

function formatAgendaMonthQuery(monthCursor: AgendaMonthCursor) {
  return monthCursor.monthKey;
}
...
function buildAgendaHref(
  ...
  const params = new URLSearchParams({
    month: formatAgendaMonthQuery(monthCursor)
  });
...
function formatAgendaAssetName(value: string) {
